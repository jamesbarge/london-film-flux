...
const FilterBar: React.FC<FilterBarProps> = ({
  ...
  const toggleCinema = (id: string, checked: boolean | string) => {
    setSelectedCinemas(prev => checked ? Array.from(new Set([...prev, id])) : prev.filter(c => c !== id));
  };
  const selectedCount = selectedCinemas.length;
  return <section aria-label="Filters" className="w-full">
        ...
        <Popover>
          ...
          <PopoverContent className="w-72 p-0" align="start">
            ...
            <div className="max-h-64 overflow-y-auto p-2">
              {isLoading && <p className="px-2 py-3 text-sm text-muted-foreground">Loading cinemas…</p>}
              {isError && <p className="px-2 py-3 text-sm text-destructive">Failed to load cinemas.</p>}
              {!isLoading && !isError && cinemas.length === 0 && <p className="px-2 py-3 text-sm text-muted-foreground">No cinemas found.</p>}
              {!isLoading && !isError && cinemas.length > 0 && <ul className="space-y-2">
                  {cinemas.map(c => {
                const checked = selectedCinemas.includes(c.id);
                return <li key={c.id} className="flex items-center gap-2 px-2">
                        <Checkbox id={`cin-${c.id}`} checked={checked} onCheckedChange={v => toggleCinema(c.id, v)} />
