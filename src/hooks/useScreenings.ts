  ...
  return useQuery<ScreeningItem[]>({
    ...
    queryFn: async () => {
      const query = (supabase as any)
        .from("screenings")
        .select("id,start_time,cinema_id,booking_url,source_url,film_id,film:films(id,title,description,letterboxd_url),cinema:cinemas(id,name)")
        .gte("start_time", from!)
        .lt("start_time", to!)
      ...
      return (data ?? []).map((row: any) => ({
        ...
        bookingUrl: row.booking_url ?? row.source_url ?? null,
        letterboxdUrl: row.film?.letterboxd_url ?? null,
      }));
    },
